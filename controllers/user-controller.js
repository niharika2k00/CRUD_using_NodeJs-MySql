import connection, { connectDB } from "../db-connection/db.js";
import { errorHandler, successHandler } from "../response-handlers/response-handler.js";
...
const addNewUser = async (req, res) => {
  try {
    ...
    let val = Math.floor((1 + Math.random()) * 0x1000000)
      .toString(16)
      .substring(1); //  .toString(16) --> means convert into hexadecimal
    ...
    if (result) res.status(200).json(successHandler(ResponseData, "User Inserted Successfully!"));
  } catch (err) {
    console.log(err);
...
const getAllUsers = async (req, res) => {
  try {
    ...
    console.log(rows);
    if (rows) res.status(200).json(successHandler(rows, "All User Fetched!"));
  } catch (err) {
    console.log(err);
...
const updateUser = async (req, res) => {
  try {
    ...
    const [result, fields] = await db.execute(query, [req.params.id, name, age || "", roll, standard, gender, address]);
    ...
    if (result) res.status(200).json(successHandler(ResponseData, "User Updated Successfully!"));
  } catch (err) {
    console.log(err);
